-- Update the transfer function to also update school_id
...
BEGIN
  ...
  -- Get new owner's school_id
  SELECT school_id INTO _new_owner_school_id
  FROM profiles
  WHERE id = _new_owner_id;
  
  -- Transfer ownership with school_id update
  UPDATE textbooks
  SET owner_id = _new_owner_id,
      school_id = _new_owner_school_id,
      status = 'available'
  WHERE id = _textbook_id;
